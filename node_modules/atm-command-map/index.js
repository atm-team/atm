...
exports.register = function (cmd) {
    cmd
        .option('-t, --target <target>', 'target', String)
        .option('-p, --parts [parts]', 'parts', split, false)
        .option('-f, --force [force]', 'force map', split, false)
        .action(function () {
            ...
            var T = {},
                target = cmd.target || 'dev',
                settings = Com.getSettings(info.projectPath),
                com = Com(info.projectPath);

            new Promise(function (resolve, reject) {
                // 校验任务名是否存在
                if (settings.destinations[target]) {
                    Com.info('will output [' + target + '] target');
                    T.getDeps = getDepsFunc(info.projectPath, target);
                    resolve();
                } else {
                    Com.exit('[' + target + '] target does not exist, please check [' + path.join(info.projectPath, 'atm-settings.js') + '] file and added in config.destinations');
                }
            })
                // 获取要发布的模块
                .then(function () {
                ...
                .then(function () {
                    ...
                    if (!versionsArr.length) {
                        ...
                        versionsArr.forEach(function (routeArr) {
                            buildVersion(routeArr.join('/'), target);
                        });
                ...
                .then(function () {
                    ...
                    function buildJsonMap(familyName, moduleName, version) {
                        var manifestPath = path.join(settings.staticPath, target, familyName, moduleName, version, 'manifest.json'),
                        ...
                        if (exportsArr.length) {
                            ...
                            // 生成json文件
                            var outputsPath = path.join(settings.mapsPath, target, familyName, moduleName);
