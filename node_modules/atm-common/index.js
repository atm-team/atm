...
Common.prototype = {
	...
	replacePlaceholder: function (str, configs, target) {
		if (!str) {
			return str;
		}
		var that = this;
		...
		configs.forEach(function (key) {
			if (defaults[key]) {
				objs[key] = defaults[key];
			} else {
				if (key === 'offset') {
					objs[key] = that.getAssetsOffset(target);
				} else if (key === 'router') {
					objs[key] = that.getAssetsRouter(target);
				}
			}

		})
		str = lib.replacePlaceholder(str, objs);
		return str || '';
	},

    // 获取静态文件位置
    getAssetsDist: function (target) {
		var that = this;
        var site = that.site;
		var siteSettings = site.settings;
		var targetConfigs = siteSettings.targets[target];
		var assetsRoot = path.join(site.getAssetsRoot(target), that.getAssetsRouter(target));
		var assetsOffset = targetConfigs.assetsOffset || '';
		assetsOffset = that.replacePlaceholder(assetsOffset, ['target', 'site', 'project', 'module', 'version', 'router'], target);
		return path.join(assetsRoot, assetsOffset);
    },
    ...
    getManifestFile: function (target) {
		...
		var siteSettings = site.settings;
		var targetConfigs = siteSettings.targets[target];
		var manifestRoot = path.join(site.getManifestsRoot(target), that.getAssetsRouter(target));
		var manifestOffset = targetConfigs.manifestsOffset || '';
		manifestOffset = that.replacePlaceholder(manifestOffset, ['target', 'site', 'project', 'module', 'version', 'router'], target);
		return path.join(manifestRoot, manifestOffset);
    },
    ...
    getMapFile: function (target) {
		...
		var siteSettings = site.settings;
		var targetConfigs = siteSettings.targets[target];
		var mapRoot = path.join(site.getMapsRoot(target), that.projectName, that.moduleName);
		var mapOffset = targetConfigs.mapsOffset || '';
		mapOffset = that.replacePlaceholder(mapOffset, ['target', 'site', 'project', 'module', 'version', 'router'], target);
		return path.join(mapRoot, mapOffset);
    },

	getAssetsOffset: function (target) {
		target = target || 'dev';
		var that = this;
		var site = that.site;
		var assetsOffset = (site.settings.targets[target] || {}).assetsOffset || '';
		return that.replacePlaceholder(assetsOffset, ['target', 'site', 'project', 'module', 'version', 'router'], target);
	},

    // 获取静态文件uri
    getAssetsUri: function (target) {
		target = target || 'dev';
		var assetsOffset = this.getAssetsOffset(target);
		if (assetsOffset) {
			return '/' + [this.getAssetsRouter(target), assetsOffset].join('/');
		} else {
    ...
    getCompleteUri: function (target) {
		return this.site.getDomain(target) + this.getAssetsUri(target);
    },
    ...
    transformId: function (id) {
        ...
        if (this._idsMap[trueId]) {
            ...
            if (trueId.indexOf(':') === -1) {
				if (trueId.indexOf('.') < 0 && trueId.indexOf('/') < 0) {
					// todo 判断是否为模块接口
					// return
				}
        ...
        arr.forEach(function (v) {
            if (semver.valid(v)) {
                ...
                if (fs.existsSync(versionPath)) {
                    var jsonPath = path.join(versionPath, 'atmjs/config.json');
    ...
    getProjectConfigs: function () {
        var projectConfigs = lib.getJson(path.join(this.projectPath, 'atmjs/config.json'));
        projectConfigs = this._transformAlias(this._transformDeps(projectConfigs));
        return projectConfigs;
    },

    getVersionConfigs: function () {
        var versionConfigs = lib.getJson(path.join(this.versionPath, 'atmjs/config.json'));
        versionConfigs = this._transformAlias(this._transformDeps(versionConfigs));
        return versionConfigs;
    },

    // 解析 atmjs/config.json 里面的deps字段里面的{{project}} 和 {{module}}
