...
Common.prototype = {
    ...
    getConfigs: function () {
		...
		var site = this.site;
		var defaultSiteConfigs = site.getDefaultConfigs();
		var userSiteConfigs = site.getUserConfigs();
		var defaultVersionConfigs = defaults;
        var userVersionConfigs = this._getVersionConfigs();

        if (!this.underLib) {
            // var siteConfigs = this.getSiteConfigs();
            var projectConfigs = this._getProjectConfigs();
            //var moduleConfigs = lib.getJson(path.join(this.modulePath, 'config.json'));

            delete defaultSiteConfigs.exports;
			delete userSiteConfigs.exports;
            delete projectConfigs.exports;

            delete defaultSiteConfigs.packs;
			delete userSiteConfigs.packs;
            delete projectConfigs.packs;
            //delete moduleConfigs.packs;

            versionConfigs = extend(true, {}, defaultSiteConfigs, defaultVersionConfigs, userSiteConfigs, projectConfigs, versionConfigs);
			return this.__configs = versionConfigs;
        } else {
			return this.__configs = extend(true, {}, defaults, versionConfigs);
		}
    ...
    getAssetsPath: function (target) {
        ...
        if (target === 'dev') {
            assetsPath = path.join(assetsPath, 'dist');
        }
        return assetsPath;
    },
    ...
    getManifestFile: function (target) {
        ...
        if (target === 'dev') {
            manifestPath = path.join(manifestPath, 'dist/manifest');
        }
    ...
    getMapFile: function (target) {
        ...
        if (target === 'dev') {
            mapPath = path.join(mapPath, 'dist/map');
        }
    ...
    getBuilds: function () {
        ...
        var projectConfigs = this._getProjectConfigs();
        var versionConfigs = this._getVersionConfigs();
        ...
        arr.forEach(function (v) {
            if (semver.valid(v)) {
                ...
                if (fs.existsSync(versionPath)) {
                    var jsonPath = path.join(versionPath, 'config.json');
    ...
    _getProjectConfigs: function () {
        var projectConfigs = lib.getJson(path.join(this.projectPath, 'config.json'));
        projectConfigs = this._transformAlias(this._transformDeps(projectConfigs));
        return projectConfigs;
    },

    _getVersionConfigs: function () {
        var versionConfigs = lib.getJson(path.join(this.versionPath, 'config.json'));
        versionConfigs = this._transformAlias(this._transformDeps(versionConfigs));
        return versionConfigs;
    },

    // 解析 config.json 里面的deps字段里面的{{project}} 和 {{module}}
